    ...
    public partial class TcpServer
    {
        ...
        public async Task HandleCreateTradeAsync(CustomHttpContext context)
        {
            ...
            {   //jwt context
                ...
                // Deserialisiert Trade-Informationen aus dem Request
        ...
        public async Task HandleExecuteTradeAsync(CustomHttpContext context)
        {
            ...
            { //jwt context auth
                var username = context.Items["Username"]?.ToString();
                Console.WriteLine($"Execute trade request from user: {username}"); //url
                ...
                var pathParts = context.Request.Path.Split('/'); // extrahiert trade aus url
                ...
                try
                {
                    // Erste Methode: Versucht direkt als Integer zu parsen
                    offeredCardId = int.TryParse(context.Request.Body.Trim(), out int parsed) 
                        ? parsed    // Wenn erfolgreich, nimm den geparsten Wert
                        : JsonSerializer.Deserialize<int>(context.Request.Body);  // Wenn nicht, versuche JSON zu parsen
        ...
        public async Task HandleDeleteTradeAsync(CustomHttpContext context)
        {
            ...
            {   //jwt authentication 
                ...
                var pathParts = context.Request.Path.Split('/'); //trade aus url
                if (pathParts.Length < 3 || !int.TryParse(pathParts[2], out int tradeId))
                {
                    context.Response.StatusCode = 400;
                    context.Response.StatusDescription = "Bad Request";
                ...
                // überprüfung ob user den trade erstellt hat
                if (trade.UserId != user.Id)
                {
                    context.Response.StatusCode = 403;
                    context.Response.StatusDescription = "Forbidden";
