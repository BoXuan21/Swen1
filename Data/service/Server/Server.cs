    ...
    public partial class TcpServer
    {
        ...
        public void Start()
        {
            ...
            while (true)
            {
                ...
                // Handles each client in a separate thread from the thread pool
        ...
        public async void HandleClient(object obj)
        {
            ...
            try
            {
                ...
                {
                    int remainingBytes = contentLength - (bytesRead - headerEndPosition - 4); // berechnet was noch gelesen werden muss
                ...
                ms.Position = 0;
                string fullRequest = Encoding.UTF8.GetString(ms.ToArray()); // bereitet die verarbeitung des lesens vor
                string[] requestLines = fullRequest.Split("\r\n");

                if (requestLines.Length < 1) //validierung
                {
                    await HttpUtils.SendErrorResponseAsync(networkStream, 400, "Invalid request format");
                    return;
                }

                string[] requestLine = requestLines[0].Split(' '); // pfad und methode
                ...
                var context = new CustomHttpContext(); //httpcontext wird erstellt
        ...
        private async Task HandleSpecialEndpoints(CustomHttpContext context, string method, string path)
        {
            ...
            {
                string tradeId = path.Split('/')[2]; // url wird extrahiert
                switch (method)
                {
                    case "POST":
